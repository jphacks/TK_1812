...
namespace Map
{
    public class CameraContoroller : MonoBehaviour
    {
        ...
        public float minCameraDistance = -200f;
        ...
        void Update()
        {
            ...
            else if (Input.touchCount == 2)
            {
                ...
                else if (Input.touches[0].phase == TouchPhase.Moved || Input.touches[1].phase == TouchPhase.Moved)
                {
                    ...
                    Debug.Log(camera.transform.localPosition.z);
                    if(cameraPosZ <= maxCameraDistance && cameraPosZ >= minCameraDistance)
                    {
                        camera.transform.localPosition = new Vector3(camera.transform.localPosition.x, camera.transform.localPosition.y, cameraPosZ);
                    }
                    else if(cameraPosZ < minCameraDistance)
                    {
                        camera.transform.localPosition = new Vector3(camera.transform.localPosition.x, camera.transform.localPosition.y, minCameraDistance);
                    }
                    else if(cameraPosZ > maxCameraDistance)
                    {
                        camera.transform.localPosition = new Vector3(camera.transform.localPosition.x, camera.transform.localPosition.y, maxCameraDistance);
                    }
                }
